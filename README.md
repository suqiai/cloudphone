...
Starting from `v0.0.10`, the package also ships with a built-in skill, `openclaw-cloudphone`, which helps agents combine these tools in a more reliable way.
...
Starting from `v0.0.10`, the package also includes an **OpenClaw skill**:
...
Starting from `v0.0.10`, the package ships with the `openclaw-cloudphone` skill. It teaches the agent to use the tools in a short loop: observe -> act -> verify -> observe again. Make sure you installed a recent version and restarted the Gateway so the latest skill was loaded.
...
The agent should call `cloudphone_render_image` automatically to turn that URL into a displayable image. If it does not, ask the agent to show the screenshot explicitly.
